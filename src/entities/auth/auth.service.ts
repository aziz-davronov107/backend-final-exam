...
export class AuthService {
  ...
  async getToken(id: number,isTrue?:boolean) {
    let isRefresh = {secret: process.env.JWT_REFRESH_SECRET || 'yandiev',expiresIn: '7d',};
    if(!isTrue){
      return {
  ...
  async register(payload: CreateDto) {
    await this.verificationService.checkConfigOtp({type:EverifationsTypes.REGISTER,...payload})
  ...
  async login(payload: LoginDto) {
    ...
    if(isValid){
      throw new HttpException('Phone or password Incorect', HttpStatus.BAD_REQUEST)
    } 
